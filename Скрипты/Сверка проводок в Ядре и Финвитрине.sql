WITH t11
        AS (SELECT                                         /*+ no_index(trn)*/
                  *
              FROM dwh.transaction_htran trn
             WHERE deleted_flag = 'N'
                   AND validto = TO_DATE ('31.12.5999', 'DD.MM.YYYY')
                   AND value_day BETWEEN CASE
                                            WHEN TO_CHAR (SYSDATE, 'D') IN
                                                    (5, 6, 7)
                                            THEN
                                               TRUNC (SYSDATE, 'IW') - 4
                                            ELSE
            ...
            SELECT   t4.value_day,
                     ...
                     SUM (
                        CASE
                           ...
                           THEN
                              -acc_cur_amt
                           WHEN (ncode >= 70614 AND ncode < 70700 AND AP = 'C')
                           THEN
                              -acc_cur_amt
                           WHEN (ncode >= 70614 AND ncode < 70700 AND AP = 'D')
                           THEN
                              acc_cur_amt
   ...
   SELECT DISTINCT
     ...
     FROM (  SELECT value_day,
                    ...
                    MAX (CASE WHEN delta <> 0 THEN 1 ELSE 0 END) AS err_flag
               FROM (  SELECT NVL (value_day5, value_day6) AS value_day,
                         ...
                         FROM (SELECT t5.value_day AS value_day5,
                                      ...
                                      t6.summa_pl,
                                      ABS (
                                         NVL (t5.summa, 0) - NVL (t6.summa_pl, 0))
                                         AS delta
                                 FROM    t5 t5
                                      FULL JOIN
                                         t6 t6
