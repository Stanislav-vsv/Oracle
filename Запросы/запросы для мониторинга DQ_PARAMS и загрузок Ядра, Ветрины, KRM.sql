SELECT * FROM DQ_PARAMS;
 ...
 SELECT *  FROM MON_INFA_WORKFLOWS   -- ФВ, наличие записей в таблице за неделю
 WHERE SERVER_NAME='INT_DW_PROD'
 AND WORKFLOW_NAME='WF_CTL_DMFR_COMP_CORR_LOADING_AW'
 AND START_TIME>=SYSDATE-7 
 AND STATUS<>'Unscheduled';
  
 SELECT *  FROM MON_INFA_WORKFLOWS   -- ФВ, наличие записей в таблице за месяц
 WHERE SERVER_NAME='INT_DW_PROD'
